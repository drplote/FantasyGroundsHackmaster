...
function handleApplyDamage(msgOOB)
  ...
  if rTarget then
    ...
    aDice = JSON.decode(msgOOB.jsonDice) or "";
  end
...
function onDamage(rSource, rTarget, rRoll)
  ...
  local nTotal = ActionsManager.total(rRoll);
    ...
    -- local aEffects = EffectManager5E.getEffectsByType(rSource, "MATH", {}, nil);
    -- if #aEffects > 0 then
      -- local sOriginal = aEffects[1].original;
      -- local sMath = StringManager.trim(sOriginal:match(":(.*)$"));
      -- if sMath then
        -- nTotal = math.floor(StringManager.evalDiceMathExpression(nTotal .. sMath))
-- Debug.console("manager_action_damage.lua","onDamage","nTotal",nTotal);   
  ...
  notifyApplyDamage(rSource, rTarget, rRoll.bTower, rMessage.text, nTotal, rRoll.aDice);
end
...
function hasBaseDamageTypes(aDmgTypes)
	...
	for _,sDmgType in pairs(aDmgTypes) do
		if sDmgType == "bludgeoning" then
			bBludgeoning = true;
		elseif sDmgType == "slashing" then
			bSlashing = true;
		elseif sDmgType == "piercing" then
			bPiercing = true;
		end
	end
...
function checkDiceReductionType(aReduction, aDmgType,aDice,nLimit)
  ...
  local nAdjust = 0;
  for _,sDmgType in pairs(aDmgType) do
    nAdjust = nAdjust + checkDiceReductionTypeHelper(aReduction[sDmgType], aDmgType,aDice);
    nAdjust = nAdjust + checkDiceReductionTypeHelper(aReduction["all"], aDmgType,aDice);
  end
  return nAdjust;
end
...
--Debug.console("manager_action_damage.lua","getDamageAdjust","rDamageOutput",rDamageOutput);    
  ...
  local aDamageDiceResist = getReductionType(rSource, rTarget, "DDR");
...
--Debug.console("manager_action_damage.lua","getDamageAdjust","aSrcDmgClauseTypes",aSrcDmgClauseTypes);    
    ...
    local bLocalDamageDiceResist = (checkNumericalReductionType(aDamageDiceResist, aSrcDmgClauseTypes) ~= 0);
--Debug.console("manager_action_damage.lua","getDamageAdjust","bLocalDamageDiceResist",bLocalDamageDiceResist); 
    ...
    if bLocalImmune then
      ...
      if bLocalDamageDiceResist then
        local nLocalDamageDiceResist = checkDiceReductionType(aDamageDiceResist, aSrcDmgClauseTypes, aDice);
...
--Debug.console("manager_action_damage.lua","applyDamage","aDice",aDice); 
  -- Get health fields
  local sTargetType, nodeTarget = ActorManager.getTypeAndNode(rTarget);
  if sTargetType ~= "pc" and sTargetType ~= "ct" then
    return;
