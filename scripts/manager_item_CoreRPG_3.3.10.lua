...
function handleParcelTransfer (msgOOB)
    ...
    if bNPC then
      if (string.match(nodeParcel.getPath(),"^combattracker")) then
        local aItemNodes = UtilityManagerADND.getItemNodeListFromNPCinCT(nodeParcel);
        for _,sItemNode in pairs(aItemNodes) do 
          local nodeItem = DB.findNode(sItemNode);
          if nodeItem then
            handleItem(nodeTargetRecord, nil, "item", nodeItem.getNodeName(), true);
          end
        end
      else
