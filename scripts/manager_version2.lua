...
function updateToNewMemorizationTotalHelper(nodeChar)
    ...
    for _,nodePower in pairs(DB.getChildren(nodeChar, "powers")) do
      ...
      if nMemorized > 0 then
        Debug.console("manager_version2.lua","updateToNewMemorizationTotalHelper","Updating memorization/totals for ",sName, "For sNameSpell",sNameSpell);
  ...
  for _,nodeChar in pairs(DB.getChildren("charsheet")) do
    for _,nodeClass in pairs(DB.getChildren(nodeChar, "classes")) do
      ...
      if nameClass == 'Ranger' then
        Debug.console("manager_version2.lua","updateClassPowerAndSkills","Checking ranger skills.",DB.getValue(nodeChar,"name",""));
        for _,nodeSkill in pairs(DB.getChildren(nodeChar, "skilllist")) do
          ...
          if nameSkill:lower() == 'hide in shadows' then
            Debug.console("manager_version2.lua","updateClassPowerAndSkills","Hide in Shadows updated.");
            DB.setValue(nodeSkill,"name","string",'Ranger: Hide in Shadows');
          end
          if nameSkill == 'Move Silently' then
            Debug.console("manager_version2.lua","updateClassPowerAndSkills","Move Silently updated.");
            DB.setValue(nodeSkill,"name","string",'Ranger: Move Silently');
          end
        end
      end
      if nameClass == 'Paladin' then
        Debug.console("manager_version2.lua","updateClassPowerAndSkills","Checking paladin powers.",DB.getValue(nodeChar,"name",""));
        for _,nodePower in pairs(DB.getChildren(nodeChar, "powers")) do
          local sNameSpell = DB.getValue(nodePower,"name","");
          if sNameSpell:lower() == 'lay on hands' then
            Debug.console("manager_version2.lua","updateClassPowerAndSkills","Lay on Hands updated.");
...
function updateCharacterClassBonusEXPField()
  for _,nodeChar in pairs(DB.getChildren("charsheet")) do
Debug.console("manager_version2.lua","updateCharacterClassBonusEXPField","Updating bonus XP value for :",DB.getValue(nodeChar,"name",""));  
    for _,nodeClass in pairs(DB.getChildren(nodeChar, "classes")) do
